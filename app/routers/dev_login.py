...
a Merces deployed on a *different* domain (a preview host like `dev1.tuckers-workshop.xyz`).
...
async def dev_login(
    ...
    """`/dev-login?key=<DEV_LOGIN_SECRET>&code=<member_code>&groups=merces-admin` → sets a
