...
locally with the shared `sso_secret` — no callback to Legion. Merces is a *consumer*: it
...
`/admin` is gated on the `merces-admin`/`merces-manager` slugs being present in `groups`;
...
ADMIN_GROUP = "merces-admin"
MANAGER_GROUP = "merces-manager"
...
def make_authorize_url(request: Request, *, return_to: Optional[str] = None) -> str:
    ...
    return f"{settings.legion_base_url}/sso/authorize?app=merces&return_to={quote(target, safe='')}"


def logout_url(request: Request, *, return_to: str = "/me") -> str:
    """Legion's single-logout endpoint, returning to `return_to` (a Merces path)."""
