...
REQUIRED_TABLES = {"members", "transactions", "store_items", "redemptions"}
...
def stage_restore(upload_bytes: bytes) -> tuple[bool, str]:
    ...
    if not validate_sqlite_file(tmp):
        os.remove(tmp)
        return False, "That file is not a valid Merces database backup."
...
def nightly_backup() -> Optional[str]:
    ...
    if not is_sqlite():
        return None
    name = f"merces-{datetime.now():%Y%m%d-%H%M%S-%f}.db"
