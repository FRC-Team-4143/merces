...
async def test_dev_login_is_not_mounted_without_a_secret(client):
    ...
    assert not settings.dev_login_secret
    resp = await client.get("/dev-login?key=whatever&code=x&groups=merces-admin")
...
def test_dev_login_handler_checks_the_secret_and_signs_a_real_cookie(monkeypatch):
    ...
    ok = tc.get("/dev-login?key=let-me-in&code=abc123&groups=merces-admin,merces-manager",
                follow_redirects=False)
    assert ok.status_code == 303
    claims = read_sso_token(ok.cookies["mw_sso"])
    assert claims["member_code"] == "abc123"
    assert set(claims["groups"]) == {"merces-admin", "merces-manager"}
